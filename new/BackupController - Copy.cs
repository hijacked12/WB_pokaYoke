...
namespace WB.Controllers
{
    public class CategoryController : Controller
    {
        ...
        [Authorize]
        public IActionResult List_Table()
        {
            IEnumerable<Category> objCategoryList = _db.Categories1;
            return View(objCategoryList);
        }
        ...
        public IActionResult LogView(string SearchString)
        {
            var enumDictionary = new Dictionary<string, IEnumerable<ScanLog>>();
            var distinctValues = _db.ScanLog.Select(x => x.Line).Distinct().ToList();
            
            IEnumerable<ScanLog> objCategoryList = _db.ScanLog;
            IEnumerable<ScanLog> objCategoryList1 = _db.ScanLog
                                                    .GroupBy(x => x.Line)
                                                    .Select(g => g.First());

            enumDictionary.Add("Enumerable1", objCategoryList);
            enumDictionary.Add("Enumerable2", objCategoryList1);

            return View(enumDictionary);
        }
        ...
        public IActionResult CancelFilter()
        {
            ...
            return View("LogView", enumDictionary);
        }

        [Authorize(Roles = "Admin")]
        public IActionResult SearchLogView(string MaterialNumber, string Status, string Supplier, string Line, DateTime? startDate, DateTime? endDate)
        {
            ...
            var enumDictionary = new Dictionary<string, IEnumerable<ScanLog>>();
            IEnumerable<ScanLog> objCategoryList = searchResults.AsEnumerable();
            IEnumerable<ScanLog> objCategoryList1 = _db.ScanLog
                                                    .GroupBy(x => x.Line)
                                                    .Select(g => g.First());
            enumDictionary.Add("Enumerable1", objCategoryList);
